...
SELECT MF.CATEGORY, MF.MAX_PRICE, FP.PRODUCT_NAME
FROM (SELECT CATEGORY, MAX(PRICE) AS MAX_PRICE
      FROM FOOD_PRODUCT
      WHERE CATEGORY IN ('과자', '국', '김치', '식용유')
      GROUP BY CATEGORY) MF JOIN FOOD_PRODUCT FP
                                 ON (MF.CATEGORY = FP.CATEGORY) AND (MF.MAX_PRICE = FP.PRICE)
ORDER BY MF.MAX_PRICE DESC;