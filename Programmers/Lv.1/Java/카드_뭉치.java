...
/*
    정확성  테스트
        ...
        테스트 13 〉	통과 (0.03ms, 85.5MB)
        테스트 14 〉	통과 (0.02ms, 71.2MB)
        테스트 15 〉	통과 (0.02ms, 78.2MB)
        테스트 16 〉	통과 (0.02ms, 76.9MB)
        테스트 17 〉	통과 (0.02ms, 76.6MB)
        테스트 18 〉	통과 (0.02ms, 74MB)
        테스트 19 〉	통과 (0.02ms, 78.4MB)
        테스트 20 〉	통과 (0.04ms, 80.5MB)
        테스트 21 〉	통과 (0.02ms, 74.9MB)
        테스트 22 〉	통과 (0.03ms, 78.2MB)
        테스트 23 〉	통과 (0.04ms, 74.8MB)
        테스트 24 〉	통과 (0.02ms, 76.9MB)
...
class Solution {
    public String solution(String[] cards1, String[] cards2, String[] goal) {
        int index1 = 0; // 첫 번째 카드 뭉치의 인덱스
        int index2 = 0; // 두 번째 카드 뭉치의 인덱스
        int size1 = cards1.length; // 첫 번째 카드 뭉치의 카드 개수
        int size2 = cards2.length; // 두 번째 카드 뭉치의 카드 개수

        for (int g = 0, len = goal.length; g < len; g++) { // 원하는 단어가 각 카드 뭉치의 검사할 순서의 카드에 적혀 있는지 검사
            if (index1 < size1 && goal[g].equals(cards1[index1])) {
                index1 += 1;
            }
            else if (index2 < size2 && goal[g].equals(cards2[index2])) {
                index2 += 1;
            }
            else {
                return "No";
