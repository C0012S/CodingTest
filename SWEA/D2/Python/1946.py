...
for t in range(1, T + 1):
    ...
    """
    # 위 코드가 아래 코드일 경우 Test Case 중 4 개 맞아서 Fail
    # 첫 번째 케이스가 끝나고 두 번째 케이스를 입력할 때 N의 입력이 줄 띄우기가 되지 않은 채 마지막 Ci[k] 바로 옆에 입력되기 때문이다.
    for k in range(len(Ki)):
        for i in range(Ki[k]):
            if width != 9:
                print(Ci[k], end='')
