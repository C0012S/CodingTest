/*
Easy. #3570 - Find Books with No Available Copies

    Accepted  5,175 / 9.4K    Acceptance Rate  54.8%
...
SELECT LB.book_id, LB.title, LB.author, LB.genre, LB.publication_year,
       (SELECT COUNT(*)
        ...
        WHERE (BR.return_date IS NULL) AND (BR.book_id = LB.book_id)) AS current_borrowers
FROM library_books LB
WHERE LB.total_copies - (SELECT COUNT(*)
                         FROM borrowing_records BC
                         WHERE (BC.return_date IS NULL) AND (BC.book_id = LB.book_id)) = 0
