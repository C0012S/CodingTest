/*
11945. Bronze 4 - 뜨거운 붕어빵

    시간 제한	    메모리 제한        제출        정답	      맞힌 사람	    정답 비율
    1 초	    32 MB            9430	    5825      5263	         64.561%
...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        int N = Integer.parseInt(token.nextToken()); // 행의 개수
        int M = Integer.parseInt(token.nextToken()); // 열의 개수

        StringBuilder sb = new StringBuilder();
        for (int n = 0; n < N; n++) {
            String breadStr = bf.readLine();

            for (int m = 0; m < M; m++) {
                sb.append(breadStr.charAt(M - m - 1));
            }
