/*
2774. Bronze 2 - 아름다운 수

    시간 제한	    메모리 제한        제출        정답	      맞힌 사람	    정답 비율
    1 초	    128 MB           4424	    3078      2816	         71.255%
...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        for (int t = 0; t < T; t++) {
            String X = bf.readLine(); // 아름다운 정도를 알고 싶은 수 (1 ≤ X ≤ 1000000000)

            int numberArr[] = new int[10];
            int answer = 0;
            for (int i = 0, len = X.length(); i < len; i++) {
                int nowNumber = X.charAt(i) - '0';
                if (numberArr[nowNumber] == 0) {
                    answer += 1;
                }

                numberArr[nowNumber] += 1;
            }
