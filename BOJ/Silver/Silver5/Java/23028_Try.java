...
// 코드 길이 : 3223B
// 해당 학기에 들을 수 있는 최대 전공 학점을 듣고 들을 수 있는 비전공 학점을 잘못 계산했다.
// 한 학기에 들을 수 있는 최대 과목은 6 개이다.
// 전공 과목을 최대로 듣고 해당 학기에 개설된 비전공 과목 개수가 (6 - 해당 학기에 개설된 전공 과목 개수)보다 크다면 (6 - 해당 학기에 개설된 전공 과목 개수)를 전공 과목을 최대로 듣고 들을 수 있는 비전공 과목 개수로 설정해야 한다.
// 그렇지 않을 경우(작거나 같다면), 해당 학기에 개설된 비전공 과목 개수를 전공 과목을 최대로 듣고 들을 수 있는 비전공 과목 개수로 설정해야 한다.
// 오답
    ...
    public static void calculateCredit() { // 8 학기 동안 들을 수 있는 학점을 계산하여 졸업이 가능한지 검사하는 함수
        ...
        if (A >= 66 && B >= 130) { // 이미 현재 학기에서 졸업 조건(전공 학점 66 점 이상, 총 학점 130 점 이상)을 만족했다면
            ...
            for (int r = 0; r < restSemester; r++) {
                ...
                int nowNotMajorCredit = (6 - notMajorNum[r]) * 3; // 해당 학기에 들을 수 있는 최대 전공 학점을 듣고 들을 수 있는 비전공 학점
