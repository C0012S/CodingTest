...
public class Main {
    ...
    static int checkSafeNum; // 간식을 받을 수 있는지 검사할 번호

    public static void canGet() { // 대기 공간에 서 있는 학생이 간식 받는 곳으로 이동할 수 있는지 검사하는 함수
        while (!waitStack.isEmpty()) { // 대기 공간이 비어 있지 않을 동안
            if (checkSafeNum != waitStack.peek()) { // 다음 간식 받을 순서(검사 완료한 간식 받을 순서의 다음 순서)가 간식 받는 곳으로 오지 않을 경우
                return;
            }

            // 다음 간식 받을 순서(검사 완료한 간식 받을 순서의 다음 순서)가 간식 받는 곳으로 올 경우
            waitStack.pop(); // 대기 공간에 서 있는 학생을 간식 받는 곳으로 이동시킨다.
            checkSafeNum += 1; // 다음 간식 받을 순서를 검사하도록 순서 값 증가
        }
    }
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int n = 0; n < N; n++) { // 승환이의 앞에 서 있는 학생들을 모두 검사
            ...
            if (studentNum == checkSafeNum) { // 학생의 번호표에 적힌 순서와 다음에 간식을 받아야 할 순서가 같을 경우
                checkSafeNum += 1; // 다음 간식 받을 순서를 검사하도록 순서 값 증가
                canGet(); // 대기 공간에 서 있는 학생이 간식 받는 곳으로 이동할 수 있는지 검사
            }
